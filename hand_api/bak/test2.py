...
def process_hand_video(output_image_dir, enable_visualization=False):
    ...
    data_dict = {
        'action': action_frames_array,
        'observation': {
            'state': state_frames_array
        },
    ...
    print(f"- action shape: {data_dict['action'].shape}")
    print(f"- observation.state shape: {data_dict['observation']['state'].shape}")
    print(f"- timestamp length: {len(data_dict['timestamp'])}")
