...
def process_hand_video(output_image_dir, enable_visualization=False):
    ...
    mano_data = torch.cat([root_trans[k:], root_rot[k:], hand_pose[k:], hand_betas[k:]], dim=-1)
    ...
    # Save the interpolated images as a video to see the effect
    def save_images_as_video(img_files, output_path, fps=30):
        """
        ...
        """
        if not img_files:
            print("No image files to save as video")
            return False
            
        # Read the first image to get dimensions
        first_img = cv2.imread(img_files[0])
        if first_img is None:
            print(f"Could not read image: {img_files[0]}")
            return False
        ...
        for img_path in tqdm(img_files, desc="Creating video"):
    ...
    video_output_path = os.path.join(seq_folder, "interpolated_video.mp4")
    save_images_as_video(imgfiles_interp, video_output_path, fps=30)
    
    # 使用插值后的数据
    action_frames_array = state_frames_interp.numpy()
    action_frames_array = action_frames_interp.numpy()
    mano_frames_array = mano_data.numpy()
    #print(mano_frames_array.shape)
    print("mano")
    for i in range(min(5, mano_frames_array.shape[0])):
        print(i, mano_frames_array[i])
    
    print("state",action_frames_array.shape)
    print("action",action_frames_array.shape)

process_hand_video("/home/admin123/projects/lerobot-plus/lerobot-joycon_plus_data/datasets/pick_put_hand/images/observation.images.webcam/episode_000000", enable_visualization=False)