...
def process_hand_video(output_image_dir, enable_visualization=False, new_base_path=None, episode_name=None):
    # If file_path is not provided, use output_image_dir as file_path
    ...
    if enable_visualization:
        ...
        output_pth = os.path.join(new_base_path, "videos", "chunk-000", "observation.images.webcam")
        print(f"Visualization output path: {output_pth}")
        ...
        run_vis2_on_video_cam_random_color(left_dict, right_dict, output_pth, img_focal, image_names, 
                            ...
                            t_w2c=torch.zeros(len(image_names),3), valid_hand=valid_hand, v=None, video_name=episode_name)
    ...
    data_dict = {
        ...
        "frame_index": list(range(len(imgfiles_interp))),
        "episode_index": [0] * len(imgfiles_interp),
        "index": list(range(len(imgfiles_interp))),
        "task_index": [0] * len(imgfiles_interp),
    }
    # 创建data/chunk-000目录
    chunk_dir = os.path.join(new_base_path, 'data', 'chunk-000')
    os.makedirs(chunk_dir, exist_ok=True)
    
    # 创建DataFrame并保存到chunk目录中
    df = pd.DataFrame(data_dict)
    file_path = os.path.join(chunk_dir, f'{episode_name}.parquet')
    df.to_parquet(file_path, engine='pyarrow') # 或者 engine='fastparquet'

    print(f"DataFrame successfully saved to {file_path}")

    return len(imgfiles_interp)

# 获取所有episode文件夹
base_path = "/home/admin123/projects/lerobot-plus/lerobot-joycon_plus_data/datasets/pick_put_hand_v1"
img_path = f"{base_path}/images/observation.images.webcam"
new_base_path = "/home/admin123/projects/lerobot-plus/lerobot-joycon_plus_data/datasets/pick_put_hand_v1_new"
episode_folders = glob(os.path.join(img_path, "episode_*"))
task_name = "Pick up the object and put it into the bowl."
# Create a unique output directory for saving all files
os.makedirs(new_base_path, exist_ok=True)
print(f"Created output directory: {new_base_path}")

episodes = []
tasks = {"task_index": 0, "task": task_name}
info = json.load(open(os.path.join(f"{base_path}/meta/info.json")))
cnt = 0
total_ep_lens = 0
# 处理每个episode文件夹
for file_path in episode_folders:
    print(f"Processing {file_path}...")
    episode_name = file_path.split("/")[-1]
    print("episode_name", episode_name)
    ep_lens = process_hand_video(file_path, enable_visualization=True, new_base_path=new_base_path, episode_name=episode_name)
    episodes.append({"episode_index": 0, "tasks": [task_name], "length": ep_lens})

    cnt += 1
    total_ep_lens += ep_lens

info["total_episodes"] = cnt
info["total_frames"] = total_ep_lens
info["total_videos"] = cnt

json.dump(info, open(os.path.join(new_base_path, "meta", "info.json"), "w"), indent=4)
