...
def run_slam(imagedir, masks, calib=None, depth=None, stride=1,  
    ...
    """ Maksed DROID-SLAM with Camera Jitter Support """
    droid = None
    depth = None
    args.filter_thresh = filter_thresh
    args.disable_vis = disable_vis
    print(f"SLAM参数: filter_thresh={filter_thresh}, camera_jitter={enable_camera_jitter}")
    masks = masks[::stride]
    ...
    if enable_camera_jitter:
        ...
        if os.path.exists(config_path):
            ...
            print(f"启用相机抖动以增强SLAM关键帧 (配置: {config_path})")
        else:
            # 回退到默认参数
            jitter_tool = CameraJitter(
            ...
            print("启用相机抖动以增强SLAM关键帧 (使用默认参数)")
    else:
        jitter_tool = None

    virtual_frame_counter = 0
    for (t, image, intrinsics) in tqdm(image_stream(imagedir, calib, stride), desc="处理SLAM帧"):
        ...
        # 添加虚拟抖动帧
        if jitter_tool is not None and t < len(img_msks) - 1:  # 不为最后一帧添加虚拟帧
            # 使用配置的生成间隔，如果配置不可用则使用每2帧
            ...
            if t % generation_interval == 0:
                ...
                if os.path.exists(config_path):
                    ...
                    num_virtual = np.random.randint(2, 4)  # 默认2-3个虚拟帧
                ...
                for i, (jittered_image, jittered_intrinsics) in enumerate(virtual_frames):
                    ...
                    virtual_t = t + 0.1 + i * 0.1  # 使用小数时间戳避免冲突
                    ...
                    try:
                        ...
                        print(f"虚拟帧 {virtual_t} 处理失败: {e}")
                        continue

    print(f"总共添加了 {virtual_frame_counter} 个虚拟帧")
