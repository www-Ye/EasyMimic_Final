...
from model.data.dataset import LeRobotMixtureDataset, LeRobotSingleDataset
from model.data.schema import EmbodimentTag
from model.experiment.data_config import DATA_CONFIG_MAP
from model.experiment.runner import TrainRunner
from model.model.gr00t_n1 import GR00T_N1_5
from model.model.transforms import EMBODIMENT_TAG_MAPPING
from model.utils.peft import get_lora_model
...
class ArgsConfig:
    ...
    individual_transforms: bool = True
    """Used in LeRobotMixtureDataset. If True, each dataset uses its own metadata for transforms; if False, use merged metadata (default: False)"""
...
def main(config: ArgsConfig):
    ...
    print(f"len(config.dataset_path): {len(config.dataset_path)}")
    ...
    if len(config.dataset_path) == 1:
        train_dataset = LeRobotSingleDataset(
            ...
            embodiment_tag=embodiment_tags[0],  # This will override the dataset's embodiment tag to "new_embodiment"
            video_backend=config.video_backend,
        )
        print(f"Loaded {config.dataset_path[0]}")
        print(f"embodiment_tag: {train_dataset.metadata.embodiment_tag}")
    else:
        single_datasets = []
        
        if len(config.embodiment_tags) != len(config.dataset_path):
            raise ValueError(
                f"Number of embodiment_tags ({len(config.embodiment_tags)}) "
                f"does not match number of datasets ({len(config.dataset_path)}). "
                f"Please provide exactly {len(config.dataset_path)} embodiment tags, "
        ...
        for i, p in enumerate(config.dataset_path):
            ...
            dataset_embodiment_tag = embodiment_tags[i]
            dataset_transforms = data_config_cls.transform()  
            ...
            print(f"Loaded {p} with embodiment tag {dataset_embodiment_tag}")
        ...
        train_dataset = LeRobotMixtureDataset(
            ...
            metadata_config={
                ...
                "individual_transforms": config.individual_transforms,
                # "statistics_weights": [0.9, 0.1] if len(config.dataset_path)==2 else [0.9, 0.05, 0.05],
            },
        )
        print(f"Loaded {len(single_datasets)} datasets, with {config.dataset_path} ")

        # print the embodiment tag for each dataset
        for i, dataset in enumerate(single_datasets):
            print(f"Dataset {i}: {config.dataset_path[i]}")
            print(f"  - embodiment_tag: {dataset.metadata.embodiment_tag}")
            print(f"  - embodiment_id: {EMBODIMENT_TAG_MAPPING[dataset.metadata.embodiment_tag.value]}")
    ...
    if data_action_horizon != model.action_head.config.action_horizon:
        ...
        from model.model.action_head.flow_matching_action_head import (
