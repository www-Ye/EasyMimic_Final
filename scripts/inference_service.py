...
from model.data.embodiment_tags import EMBODIMENT_TAG_MAPPING
from model.eval.robot import RobotInferenceClient, RobotInferenceServer
from model.experiment.data_config import DATA_CONFIG_MAP
from model.model.policy import Gr00tPolicy
...
class ArgsConfig:
    ...
    # Control which video inputs are used at inference time
    video_mode: Literal["dual", "webcam"] = "dual"
    """Video inputs for inference: 'dual' (webcam+phone) or 'webcam' (webcam only)."""
...
def main(args: ArgsConfig):
    if args.server:
        ...
        # Optionally override data_config to a single-video variant based on video_mode
        selected_data_config_key = args.data_config
        if args.video_mode == "webcam":
            one_video_overrides = {
                # map dual-cam config -> single-cam variant
                "so100_plus_abs_min_max": "so100_plus_abs_min_max_one_video",
            }
            if selected_data_config_key in one_video_overrides:
                selected_data_config_key = one_video_overrides[selected_data_config_key]

        data_config = DATA_CONFIG_MAP[selected_data_config_key]
        ...
        # Start the server
        if args.http_server:
            from model.eval.http_server import HTTPInferenceServer  # noqa: F401
