...
from model.utils.video import get_all_frames, get_frames_by_timestamps
...
class LeRobotMixtureDataset(Dataset):
    ...
    The dataset supports two transform modes:
    - Merged transforms (default): All datasets use merged metadata for normalization
    - Individual transforms: Each dataset uses its own original metadata for transforms
    """
    ...
    def __init__(
        ...
        Args:
            ...
            metadata_config (dict): Configuration for metadata handling, including:
                - "percentile_mixing_method": Method to mix percentiles ("weighted_average" or "min_max")
                - "individual_transforms": If True, each dataset uses its own metadata for transforms; if False, use merged metadata (default: False)
        """
        ...
        self.seed = seed
        self.mode = mode
        self.individual_transforms = metadata_config.get("individual_transforms", False)
    ...
    def update_metadata(self, metadata_config: dict) -> None:
        ...
        Args:
            metadata_config (dict): Configuration for the metadata.
                ...
                "individual_transforms": If True, each dataset uses its own metadata for transforms; if False, use merged metadata.
        """
        ...
        for tag, metadatas in all_metadatas.items():
            print(f"tag: {tag}")
            #breakpoint()
            stats_weights = metadata_config.get("statistics_weights", None)
            if stats_weights is None:
                stats_weights = self.dataset_sampling_weights.tolist()
            else:
                assert len(stats_weights) == len(self.datasets), \
                    f"statistics_weights length {len(stats_weights)} must match num datasets {len(self.datasets)}"

            self.merged_metadata[tag] = self.merge_metadata(
                metadatas=metadatas,
                dataset_sampling_weights=stats_weights,
                percentile_mixing_method=metadata_config["percentile_mixing_method"],
            )
        # Set transforms metadata based on individual_transforms setting
        if self.individual_transforms:
            # Each dataset uses its own original metadata for transforms
            for dataset in self.datasets:
                dataset.set_transforms_metadata(dataset.metadata)
        else:
            # Use merged metadata for all datasets (original behavior)
            for dataset in self.datasets:
