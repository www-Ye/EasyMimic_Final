...
from model.data.embodiment_tags import EMBODIMENT_TAG_MAPPING, EmbodimentTag
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    # Video view control
    black_view_config: dict[str, list[str]] = Field(
        default_factory=dict,
        description="Configuration for which views to make black for specific embodiments. "
                   "Format: {'embodiment_name': ['view_name1', 'view_name2']}. "
                   "Example: {'human_hand': ['phone']} to make phone view black for human_hand embodiment."
    )
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        print(f"embodiment_tag: {self.embodiment_tag}")
        # breakpoint()
    ...
    def get_embodiment_tag(self) -> int:
        ...
        # print(f"embodiment_tag_mapping: {self.embodiment_tag_mapping[self.embodiment_tag.value]}")
    ...
    def _prepare_video(self, data: dict):
        ...
        # print(f"Video input count: {images.shape[0]} (expected: 2 for webcam + phone)")
        
        # Debug: Show expected video structure
        expected_video_structure = {
            "webcam": "index 0",
            "phone": "index 1"
        }
        # print(f"Expected video structure: {expected_video_structure}")
        
        # Apply black view transformation based on configuration
        if (self.embodiment_tag is not None and 
            self.embodiment_tag.value in self.black_view_config and 
            images.shape[0] > 1):
            
            # Get the list of views to make black for this embodiment
            views_to_black = self.black_view_config[self.embodiment_tag.value]
            
            # Use hardcoded video modality mapping for hand dataset
            # This maps view names to their expected indices in the images array
            video_modality = {
                "webcam": 0,      # First video input (webcam)
            ...
            for view_name in views_to_black:
                if view_name in video_modality:
                    ...
                    view_index = video_modality[view_name]
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        # print(f"transformed_data['embodiment_id']: {transformed_data['embodiment_id']}")
        # breakpoint()
