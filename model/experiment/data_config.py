...
from model.data.dataset import ModalityConfig
from model.data.transform.base import ComposedModalityTransform, ModalityTransform
from model.data.transform.concat import ConcatTransform
from model.data.transform.state_action import (
    StateActionPerturbation,
    StateActionSinCosTransform,
    StateActionToTensor,
    StateActionTransform,
)
from model.data.transform.video import (
    VideoColorJitter,
    VideoCrop,
    VideoResize,
    VideoToNumpy,
...
from model.model.transforms import GR00TTransform
...
class So100PlusRelDataConfig(BaseDataConfig):
    video_keys = ["video.webcam"]
    state_keys = [
        "state.single_arm_eef_xyz", 
        "state.single_arm_eef_rpy", 
        "state.gripper"
    ]
    action_keys = [
        "action.single_arm_eef_xyz", 
        "action.single_arm_eef_rpy", 
        "action.gripper"
    ]
    language_keys = ["annotation.human.task_description"]
    observation_indices = [0]
    state_indices = [0]
    action_indices = [-1] + list(range(16))
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            StateActionTransform(
                apply_to=self.state_keys,
                normalization_modes={
                    # "state.single_arm_eef_xyz": "min_max",
                    # "state.single_arm_eef_rpy": "min_max",
                    "state.gripper": "min_max",
                },
                # target_rotations={
                #     "state.single_arm_eef_rpy": "rotation_6d",
                # },
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    # "action.single_arm_eef_xyz": "min_max",
                    # "action.single_arm_eef_rpy": "min_max",
                    "action.gripper": "min_max",
                },
                # target_rotations={
                #     "action.single_arm_eef_rpy": "axis_angle",
                # },
            ...
            GR00TTransform(
                state_horizon=len(self.observation_indices),
                action_horizon=len(self.action_indices)-1,
                max_state_dim=64,
                max_action_dim=32,
            ),
        ]
...
class So100PlusAbsDataConfig(BaseDataConfig):
    video_keys = ["video.webcam", "video.phone"]
    state_keys = [
        "state.single_arm_eef_xyz", 
        "state.single_arm_eef_rpy", 
        "state.gripper"
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            StateActionToTensor(apply_to=self.state_keys),
            StateActionToTensor(apply_to=self.action_keys),
            # RandomCoordinateTransform(
            #     apply_to=('state.single_arm_eef_xyz', 'state.single_arm_eef_rpy'),
            #     apply_to_2=('action.single_arm_eef_xyz', 'action.single_arm_eef_rpy'),
            # ),
            StateActionTransform(
                apply_to=self.state_keys,
                normalization_modes={
                ...
                # target_rotations={
                #     "state.single_arm_eef_rpy": "rotDATA_CONFIG_MAPation_6d",
                # },
                action_representation="absolute",
            ),
            # StateActionPerturbation(
            ...
            GR00TTransform(
                ...
                max_state_dim=64,
                max_action_dim=32,
                black_view_config={"human_hand": ["phone"]},
            ),
        ]
        return ComposedModalityTransform(transforms=transforms)
    

class So100PlusAbsDataConfigOneVideo(BaseDataConfig):
    video_keys = ["video.webcam"]
    state_keys = [
        "state.single_arm_eef_xyz", 
        "state.single_arm_eef_rpy", 
...
class So100PlusAbsRandCoordDataConfig(BaseDataConfig):
    video_keys = ["video.webcam"]
    state_keys = [
        "state.single_arm_eef_xyz", 
        "state.single_arm_eef_rpy", 
...
class So100PlusAbsRandCoordHandDataConfig(BaseDataConfig):
    video_keys = ["video.webcam"]
    hand_keys = ["hand.mano"]
    state_keys = [
        "state.single_arm_eef_xyz", 
    ...
    def modality_config(self) -> dict[str, ModalityConfig]:
        ...
        hand_modality = ModalityConfig(
            delta_indices=self.state_indices,
            modality_keys=self.hand_keys,
        )
...
class So100PlusAbsMeanStdDataConfig(BaseDataConfig):
    video_keys = ["video.webcam", "video.phone"]
    state_keys = [
        "state.single_arm_eef_xyz", 
        "state.single_arm_eef_rpy", 
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            StateActionTransform(
                apply_to=self.state_keys,
                normalization_modes={
                    "state.single_arm_eef_xyz": "mean_std",
                    "state.single_arm_eef_rpy": "mean_std",
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    "action.single_arm_eef_xyz": "mean_std",
                    "action.single_arm_eef_rpy": "mean_std",
...
DATA_CONFIG_MAP = {
    ...
    "so100_plus_rel": So100PlusRelDataConfig(),
    "so100_plus_abs_min_max": So100PlusAbsDataConfig(),
    "so100_plus_abs_min_max_one_video": So100PlusAbsDataConfigOneVideo(),
    "so100_plus_abs_mean_std": So100PlusAbsMeanStdDataConfig(),
    "so100_plus_abs_rand_coord": So100PlusAbsRandCoordDataConfig(),
}
