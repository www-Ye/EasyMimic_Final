...
from lerobot.common.robot_devices.control_utils_gr00t import (
    control_loop,
    has_method,
...
class Gr00tRobotInferenceClient:
    ...
    def get_action(self, img_webcam, img_phone, state, action_space='abs'):
        # if 'relative' in action_space:
        #     state = state.reshape(2, -1)
        #     obs_dict = {
        #         "video.webcam": img_webcam[np.newaxis, :, :, :],
        #         "video.phone": img_phone[np.newaxis, :, :, :],
        #         "state.single_arm_eef_xyz": state[:, :3].astype(np.float64),
        #         "state.single_arm_eef_rpy": state[:, 3:6].astype(np.float64),
        #         "state.gripper": state[:, 6:7].astype(np.float64),
        ...
        if img_phone is not None:
            obs_dict = {
                "video.webcam": img_webcam[np.newaxis, :, :, :],
                "video.phone": img_phone[np.newaxis, :, :, :],
                "state.single_arm_eef_xyz": state[:3][np.newaxis, :].astype(np.float64),
                "state.single_arm_eef_rpy": state[3:6][np.newaxis, :].astype(np.float64),
        ...
        # print(obs_dict["state.single_arm_eef_xyz"].shape)
        # print(obs_dict["state.single_arm_eef_rpy"].shape)
        # print(obs_dict["state.gripper"].shape)
        # breakpoint()
...
def record(
    ...
    reset_robot_to_init_gpos(robot)
...
if __name__ == "__main__":
    ...
    parser_record.add_argument("--lang-instruction", type=str, default="", help="Language instruction for the gr00t robot.")
