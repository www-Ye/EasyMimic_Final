...
class ManipulatorRobotConfig:
    ...
    # eef_rpy 7D (x, y, z + r, p, y + gripper distance)
    # delta_eef_rpy 7D (dx, dy, dz + dr, dp, dy + gripper distance)
    # eef_aa 7D (x, y, z + rx, ry, rz + gripper distance)
    # eef_quat 7D (x, y, z + q0, q1, q2, q3 + gripper distance)
    # joint_angles 7D (j0, j1, j2, j3, j4, j5, j6)
    # joint_angles+eef_rpy ....
    # action_space: str = "joint_angles"
    action_space: str = "eef_rpy"
...
class ManipulatorRobot:
    ...
    def connect(self):
        ...
        for name in self.follower_arms:
            ...
            print("qpos", self.controllers[name].mjdata.qpos[self.controllers[name].qpos_indices])
            # breakpoint()
        for name in self.leader_arms:
    ...
    def teleop_step(
        ...
        if self.controllers is not None:
            ...
            for name in self.controllers:
                ...
                goal_pos, button_control, controller_command[name] = self.controllers[name].get_command(present_pos)   
                if name == 'right':
                    self.button_control = button_control         
                    
                    # print(f'{self.button_control=}')
                leader_pos[name] = torch.from_numpy(np.array(goal_pos))
        ...
        for name in self.follower_arms:
            ...
            target_eef_state = controller_command[name]

            # # Cap goal position when too far away from present position.
            # # Slower fps expected due to reading from the follower.
            # # if self.config.max_relative_target is not None:
            # present_pos = self.follower_arms[name].read("Present_Position")
            # current_eef_state = self.controllers[name].get_eef(present_pos, self.action_space)
            
            # # Convert quaternion to Euler angles (roll, pitch, yaw)
            # x, y, z = current_eef_state[:3]
            # quat = current_eef_state[3:]
            # # Convert quaternion to Euler angles (RPY)
            # import transforms3d
            # euler = transforms3d.euler.quat2euler(quat, 'sxyz')
            # roll, pitch, yaw = euler
            # print("当前eef********=", current_eef_state)
            # print("当前eef (xyz + rpy) =", [x, y, z, roll, pitch, yaw])
            ...
            if self.action_space == "joint_angles":
                # Used when record_data=True
                follower_goal_pos[name] = goal_pos
            elif (self.action_space == "eef_rpy") or self.action_space == "eef_aa":
    ...
    def capture_observation(self):
        ...
        sim_state = []
        for name in self.follower_arms:
            if name in sim_follower_pos:
                sim_state.append(sim_follower_pos[name])
        if len(sim_state) > 0:
            sim_state = np.concatenate(sim_state)
        else:
            sim_state = np.zeros(0)
        # print(f"sim_state: {sim_state.shape}")
        ...
        obs_dict = {}
        obs_dict["observation.state"] = state
        # obs_dict["observation.sim_state"] = sim_state
        for name in self.cameras:
            obs_dict[f"observation.images.{name}"] = images[name]
        return obs_dict, sim_state
    ...
    def send_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        for name in self.follower_arms:
            ...
            if self.action_space == "joint_angles":
                ...
                self.current_state[name] = action[from_idx:to_idx]
                goal_pos = self.controllers[name].eef_action_to_joint_angles(action[from_idx:to_idx], self.action_space)
